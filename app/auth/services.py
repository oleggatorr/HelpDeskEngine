...
class AuthService:
    ...
    async def login(self, request: LoginRequest) -> LoginResponse:
        result = await self.db.execute(select(User).where(User.login == request.login))
    ...
    async def register(self, request: RegisterRequest) -> UserResponse:
        for field, error_msg in [
            ...
            res = await self.db.execute(select(User).where(field == getattr(request, field.name)))
        ...
        profile = UserProfile(user_id=user.id, permissions={})
        self.db.add(profile)
    ...
    async def list_filtered(
        ...
        if filters:
            ...
            if filters.permissions:
                needs_profile_join = True
                conditions.append(
                    UserProfile.permissions.contains(filters.permissions)
                )
